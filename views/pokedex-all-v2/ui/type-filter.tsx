...
import { TypeBadge, TypeIcon, TypeText } from '@/entities/type/ui';
      ...
      >
        ...
        <div className="px-5 py-1 flex flex-col gap-5 overflow-auto">
          <div className="grid grid-cols-2 gap-3">
            {/* <Toggle className="w-24 h-9.5" pressed=>모든 타입</Toggle> */}
            {types.map((type) => (
              <Toggle
                key={type.identifier}
                ...
                className={cn(
                  ' flex border border-transparent justify-start bg-muted/70 gap-3.5 px-3 h-11 group rounded-4xl  aria-pressed:bg-dark aria-pressed:text-primary-foreground',
                )}
                aria-label={type.nameKo}
              >
                <TypeIcon type={type} className={cn('size-7.5 ')} />
                {/* <span className="pr-2.5">{type.nameKo}</span> */}
                <span className="flex-1 text-center"> {type.nameKo}</span>
              </Toggle>
            ))}
          </div>
        </div>
